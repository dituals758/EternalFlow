export class EternalFlowApp {
    ...
    init() {
        ...
        // Показать динамический остров установки после user interaction
        // (покажем по клику на остров, если поддерживается)
    ...
    createParticles() {
        ...
        if (!container) return;
        const particleCount = 20; // умерить для производительности
        for (let i = 0; i < particleCount; i++) {
            ...
            particle.style.background = `hsla(${hue}, 80%, 70%, ${Math.random() * 0.2 + 0.05})`;
        ...
        try {
            ...
            if (stored) {
                ...
                if (Array.isArray(parsed)) {
                    this.events = parsed.filter(e => e.id && e.title && e.date && !isNaN(new Date(e.date).getTime()));
                }
            }
        } catch (err) {
    ...
    getCorrectLabel(value, labels) {
        // Русская грамматика: год/года/лет
        if (value % 10 === 1 && value % 100 !== 11) return labels[2];
        if (value % 10 >= 2 && value % 10 <= 4 && (value % 100 < 10 || value % 100 >= 20)) return labels[1];
        return labels[0];
    }
    ...
    setupServiceWorker() {
        if ('serviceWorker' in navigator) {
            // регистрация уже вызывается в index.html после полной загрузки
            // здесь можно добавить дополнительные логики при обновлении
        }
    }

    // Допустим, отдельная функция для регистрации может быть добавлена позже
}

document.addEventListener('DOMContentLoaded', () => {
    // Создаем экземпляр приложения
    window EternalFlowAppInstance = new EternalFlowApp();
    // Глобально для доступа в консоли при необходимости
});

// Экспортируем класс для возможности тестирования в средах модулей (по желанию)
export default EternalFlowAppInstance || null;